...
static NSString *const timeControlStatus = @"timeControlStatus";
...
- (void)initMP{
    ...
    //The selectors (it would appear that iOS gets weird if you don't specify a target, even if you aren't using the selector.
    [remoteCommandCenter.previousTrackCommand addTarget:self action:@selector(onPrevious:)];
    [remoteCommandCenter.nextTrackCommand addTarget:self action:@selector(onNext:)];
    [remoteCommandCenter.playCommand addTarget:self action:@selector(onPlay:)];
    [remoteCommandCenter.pauseCommand addTarget:self action:@selector(onPause:)];
}
-(MPRemoteCommandHandlerStatus) actionDoNothing:(MPRemoteCommand *)sender {
    
    return MPRemoteCommandHandlerStatusSuccess;
}
-(MPRemoteCommandHandlerStatus) onPrevious:(MPRemoteCommand *)sender {
    if (self.onRemotePrevious) {
        self.onRemotePrevious(@{@"target": self.reactTag});
    }
    return MPRemoteCommandHandlerStatusSuccess;
}
-(MPRemoteCommandHandlerStatus) onNext:(MPRemoteCommand *)sender {
    if (self.onRemoteNext) {
        self.onRemoteNext(@{@"target": self.reactTag});
    }
    return MPRemoteCommandHandlerStatusSuccess;
}
-(MPRemoteCommandHandlerStatus) onPlay:(MPRemoteCommand *)sender {
    if (self.onRemotePlay) {
        self.onRemotePlay(@{@"target": self.reactTag});
    }
    return MPRemoteCommandHandlerStatusSuccess;
}
-(MPRemoteCommandHandlerStatus) onPause:(MPRemoteCommand *)sender {
    if (self.onRemotePause) {
        self.onRemotePause(@{@"target": self.reactTag});
    }
    return MPRemoteCommandHandlerStatusSuccess;
}
- (void)setControlCenter:(NSDictionary*)data {
    [self initMP];

    if (@available(iOS 13.0, *)) {
        _center.playbackState = MPNowPlayingPlaybackStatePlaying;
    }
    ...
    if (data[@"thumbnailURL"]) {
        ...
        NSURLSessionDataTask *dataTask = [[NSURLSession sharedSession] dataTaskWithURL:imageurl completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
            ...
            _nowPlayingInfo[MPMediaItemPropertyArtwork] = artwork;

            _center.nowPlayingInfo = _nowPlayingInfo;
        }];
        [dataTask resume];
    }
  ...
  if (object == _playerItem) {
    ...
    if ([keyPath isEqualToString:statusKeyPath]) {
      ...
      if (_playerItem.status == AVPlayerItemStatusReadyToPlay) {
        ...
        _nowPlayingInfo[MPNowPlayingInfoPropertyElapsedPlaybackTime] = [NSNumber numberWithFloat:CMTimeGetSeconds([[_player currentItem] currentTime])];
        _nowPlayingInfo[MPMediaItemPropertyPlaybackDuration] = [NSNumber numberWithFloat:CMTimeGetSeconds([[_player currentItem] asset].duration)];
        _nowPlayingInfo[MPNowPlayingInfoPropertyPlaybackRate] = [NSNumber numberWithFloat:[_player rate]];
        _center.nowPlayingInfo = _nowPlayingInfo;
    ...
    if([keyPath isEqualToString:playbackRate]) {
      ...
      if(_playbackStalled && _player.rate > 0) {
        ...
        if (@available(iOS 10.0, *)) {
            NSLog(@"timeControlStatus trigger");
